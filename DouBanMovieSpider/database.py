...
class DBManager(object):
    ...
    def isScenaristExist(self, i_id):
        con = sqlite3.connect(self.db_file)
        cur = con.cursor()
        cur.execute('''SELECT EXISTS(SELECT 1 FROM scenarist WHERE id=? LIMIT 1)''', (i_id,))
        is_exit = cur.fetchone()
        if is_exit[0] == 1:
            return True
        else:
    ...
    def isActorExist(self, i_id):
        con = sqlite3.connect(self.db_file)
        cur = con.cursor()
        cur.execute('''SELECT EXISTS(SELECT 1 FROM actor WHERE id=? LIMIT 1)''', (i_id,))
        is_exit = cur.fetchone()
        if is_exit[0] == 1:
            return True
        else:
    ...
    def isDirectorExist(self, i_id):
        con = sqlite3.connect(self.db_file)
        cur = con.cursor()
        cur.execute('''SELECT EXISTS(SELECT 1 FROM director WHERE id=? LIMIT 1)''', (i_id,))
        is_exit = cur.fetchone()
        if is_exit[0] == 1:
            return True
        else:
    ...
    def insertMoiveItem(self, mItem):
    ...
    def insertActorItem(self, dItem):
        # Add a director item to database
        con = sqlite3.connect(self.db_file)
        cur = con.cursor()
        cur.execute('''INSERT OR IGNORE INTO actor(id, name, gender, zodiac, livingTime, birthday, 
    ...
    def insertScenaristItem(self, dItem):
